  ...
  (cond
        ...
        [(list? sexp)
         (case (first sexp)
                ...
                [(and)  (opS myAnd (map parse (cdr sexp)))]
                            ...
                            (let ([sexp0 (cdr sexp)])
                                      (if (equal? (length sexp0) 2)
                                       (withS (for/list ([j (first sexp0)])
                                                 (cond
                                                    [(and (equal? 4 (length j)) (symbol? (car j)) (equal? ': (second j)))
                                                        (binding (first j) (findType (third j)) (parse (fourth j))) ]
...
;;Funcion que dada una s-expresion la convierte a algo de tipo Type
;;s-expresion -> Type
(define (findType t)
    (cond
        [(equal? 'number t) (numberT)]
        [(equal? 'boolean t) (booleanT)]
        [(list? t) (funT (for/list ([i t] #:when (not (equal? '-> i))) (findType i)))]
        [else (error 'findType "No existe un tipo asociado para ~a" t)]))
